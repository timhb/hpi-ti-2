...
Seien $x, y in RR$ zwei Variablen. Modelliere die folgenden Gleichungen als
lineares Programm oder erläutere informell warum dies nicht möglich ist: 
...
Finde eine Platzierung der Vitrinen im Raum, die alle Sicherheitsabstände
einhält und den Weg des Dozenten minimiert. Die Größe der Vitrinen ist dabei
verschwindend gering und muss nicht betrachtet werden. Modelliere das Problem
als ein lineares Programm.

*Lösung:*

Wir formalisieren zunächst, dass die Poster sich an den Vitrinen selbst
befinden und damit die Positionen der Artefakte, Vitrinen und Poster exakt
übereinstimmen und alle als Punkte betrachtet werden können. Im Folgenden
sprechen wir nur noch vom jeweiligen Artefakt. Dies ist zwar nicht explizit in
der Aufgabenstellung spezifizert, die Alternative würde jedoch den Sinn des
Optimierungsproblems verfehlen, da die Positionen der Poster sonst nicht
eingeschränkt wären. Da die Größe der Vitrinen nicht betrachtet werden muss,
gehen wir außerdem davon aus, dass sich mehrere Vitrinen am selben Punkt
befinden können.

Wir wählen ein Koordinatensystem mit Ursprung in der nord-westlichen Ecke des
Raumes, x-Achse nach Osten und y-Achse nach Süden. Seien $B,T in RR$ jeweils
die Breite und Tiefe des Raumes. Setze $R = [0,B] times [0,T]$.

Sei $n in NN$ die Anzahl an Artefakten im Raum. Für alle $i in {1,2,...,n}$ sei
$a_i = (x_i, y_i) in R^2$ die Position von Artefakt $i$ mit den
Mindestabständen $(n_i, o_i, s_i, w_i) in R^4$, also jeweils der nördliche,
östliche, südliche und westliche Abstand. O.B.d.A sind die Indizes sind so
gewählt, dass sie der gewünschten Reihenfolge des Dozenten entsprechen. Dieser
beginnt also bei Artefakt $1$ und endet bei Artefakt $n$. Zusätzlich definieren
wir Start- und Endpunkt $(x_0, y_0) = (x_(n+1),y_(n+1)) = (0,0)$. Sei ferner
für alle $i in {0,1,...,n+1}$ $u_i$ die horizontale und $v_i$ die vertikale
Laufdistanz des Dozenten zwischen zwei aufeinanderfolgenden Artefakten.
...
Der Dozent muss für zwei aufeinanderfolgende Artefakte mindestens ihren Abstand
laufen:
