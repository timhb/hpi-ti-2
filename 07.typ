...
+ "$==>$": Wir zeigen $L "hat einen endlichen Index" -> overline(L) "hat einen endlichen Index"$. \
  ...
  Angenommen $overline(L)$ hat einen endlichen Index. So gilt nach Myhill-Nerode $overline(L) in R E
  G$. Nach den Abschlusseigenschaften der regulären Sprachen gilt auch $ overline(overline(L)) = L
  in R E G$ womit $L$ nach Myhill-Nerode einen endlichen Index hat.
...
+ Sei $L = Sigma^*$ mit $Sigma = {0, 1}$ und $L' = {0} subset.eq L$. \
  ...
  _(auf $epsilon$ kann noch eine 0 folgen, auf $0$ nicht mehr, ohne dass das Wort nicht mehr in $L'$
  liegt)_. Somit gilt $i' >= 2 > 1 = i$. Dies ist ein Widerspruch zu $i' <= i$, die Aussage ist also
  falsch. #qed
...
+ Wir nehmen an, $A_N$ ist nicht der minimale DEA für die Sprache $L$. \
  ...
  $A_N$ akzeptiert nur das Eingabewort $0 in Sigma^*$. Bei allen weiteren Eingaben verfällt $A_N$ in
  $q_3$, von wo aus es keinen Weg mehr in einen akzeptierenden Zustand gibt. \
  ...
  Sei $Q_N$ die Zustände in $A_N$ und $Q_D$ die Zustände in $A_D$. So gilt $|Q_N| > |Q_D|$. Dies ist
  ein Widerspruch zu $2^(|Q_N|) >= |Q_D| >= |Q_N|$. Die Aussage ist widerlegt. #qed
...
+ Seien ${[0]_A, [1]_A, [2]_A, [3]_A, [4]_A, [5]_A, [6]_A}$ alle Äquivalenzklasse unter
  ...
  einer Äquivalenzklasse zugeordnet werden. \ Außerdem sind die Äquivalenzklassen nicht leer:
  $epsilon in [0]_1$, $1 in [1]_1$ ... . \
...
+ Angenommen es gäbe zwei Wörter $u,v in Sigma^*$ mit $u = 0^j 1$ und $v = 0^k 1$ mit o.B.d.A $j <
  ...
  Dies ist ein Widerspruch, da für $w = 0^j$, $u w in L$ und $v w in.not L$ gilt. Da $j, k$ beliebig
  sein können gilt dies für alle $j < k$, also für unendlich viele Wörter, was wiederum zu unendlich
  vielen Äquivalenzklassen führt. #qed
...
+ $L^R = {w^R | w in L}$ ist regulär. \
  Dabei ist $w^R$ die Spiegelung des Wortes $w$, d.h. für $w in Sigma^*, w_1, ..., w_(|w|) in Sigma$
  mit $w = w_1 w_2 ... w_(|w|)$ ist $w^R$ definiert als $w^R := w_(|w|) w_(|w|-1) ... w_1$.
  ...
  *Lösung:*
  ...
  Sei $w in L(M^R)$ mit Länge $n in NN$ und $w= w_1 w_2 ... w_n$. Da $w$ von $M^R$ akzeptiert wird,
  gibt es eine Berechnung $M^R (w)= (r_0, r_1, ..., r_n) in Q^(n+1)$, sodass:

  - $r_0 in F$, also Teil der Startzustände von $M^R$ (entspricht akzeptierenden Zuständen von $M$)
  ...
  - $s_0 = r_n = q_0$
  - $forall i in [n]: s_i = r_(n-i) = delta(r_(n-i+1), w_(n-i+1)) =
    delta(r_(n-(i-1)), w^R_i) = delta(s_(i-1),w^R_i)$
  - $s_n = r_0 in F$.

  $(s_0, s_1, ..., s_n)$ ist per Definition eine akzeptierende Berechnung von $M$ auf $w^R$. Es gilt
  also\
  $w^R in L(M)=L$ und damit $(w^R)^R = w in L^R$.
  ...
  - $s_0 = r_n in F$
  - $forall i in [n]: s_i = r_(n-i) in delta^(-1)(r_(n-i+1), w^R_(n-i+1)) = delta^(-1)(r_(n-(i-1)),
    w_i) = delta^(-1)(s_(i-1), w_i)$
  - $s_n = r_0 in {q_0}$.

  $(s_0, s_1, ..., s_n)$ ist per Definition eine akzeptierende Berechnung von $M^R$ auf $w$. Also $w
  in L(M^R)$.

  Da $L(M^R) subset.eq L^R$ und $L^R subset.eq L(M^R)$, gilt $L(M^R) = L^R$. Somit ist $L^R$ regulär.
  #qed
...
+ $min(L) = {w in L | "kein echtes Präfix von" w "ist in" L}$ ist regulär.
  ...
  *Lösung:*
  ...
  Sei $w in L$. Ein echter Präfix von $w$ ist genau dann in $L$, wenn es $u in L$ und $x in Sigma^+$
  gibt, sodass $w = u x$. Es gilt also $min(L) = L \\ (L compose Sigma^+)$.
  ...
  Da reguläre Sprachen unter Konkatenation und Mengendifferenz abgeschlossen sind, ist $min(L)$ also regulär.
